<?php
    ...
    $q = "select t.cleaned_name as Tournament, p2.tag as winner, p1.tag as player_1, p3.tag as player_2, s.p1_score as player1_score, s.p2_score as player2_score, s.bracket_name as bracket_name, s.best_of as best_of from melee_players as p1, melee_players as p2, melee_players as p3, melee_sets as s, melee_tournaments as t where p1.player_id = s.p1_id and p2.player_id = s.winner_id and p3.player_id = s.p2_id and s.tournament_id = t.tournament_id and ((p1.tag = ? and p3.tag = ?) or (p1.tag = ? and p3.tag = ?))";
    $query = $pdo->prepare($q);
    $query->execute([$player1,$player2,$player2,$player1]);
